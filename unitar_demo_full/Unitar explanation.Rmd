...
Using this method to install `unitar` without Rtools will results in an error. Rtools and an installation guide can be found at https://cran.r-project.org/bin/windows/Rtools/
...
First, each targets project must be run so the targets can be made and referenced. Usually, you have already run these targets previously, and in this case, this is an unnecessary step. 

```{r}
library(targets)

path_to_unitar_demo_full = "path/to/unitar_demo_full"

project_root = paste(path_to_unitar_demo_full, "/unitar_demo_full/extdata/target_projects", sep="")
project_subfolders = list.files(project_root)
project_folders = paste(project_root, project_subfolders, sep="/")

for (i in 1:4){
  setwd(project_folders[i])
  tar_make()
}
```


This particular example reads the files from the example folder itself, but project_root can be any directory that contains a list of targets projects.


Here's how you'd load these now-made targets in using `unitar_read`:
...
# _targets.R
library("unitar")
tar_dirs="../1refdata1"  # external targets projects you want to track
list(
...
list(
  tar_target(
    big_data_set_path,
    unitar_path("big_data_set", "../1refdata1"),
    format = "file"
  ),
  tar_target(
    filtered_data_set,
...
You can add or remove projects from your PEP by editing the project_config.yaml file.

```{r}
pep_config = paste(path_to_unitar_demo_full, "/unitar_demo_full/extdata/metadata/project_config.yaml", sep="")
p = pepr::Project(pep_config)
pdir = unitar::peptar_dirs(p)
ppath = unitar::peptar_path(p, "ref2")
pmeta = unitar::peptar_meta(p, fields="name")
pmake = unitar::peptar_make(p)
